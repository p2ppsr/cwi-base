...
export interface DojoClientApi extends DojoPublicApi {
    ...
    *
     * @param state generated by `copyState`
     * @param makeChanges if true, updates and inserts are applied to database and returns records inserted and original values for records updated.
     * @returns updates equal to the number of records requiring updates. inserts equal to the number of new records requiring inserts.
     */
    mergeState(state: DojoUserStateApi, makeChanges?: boolean) : Promise<{ updates: number, inserts: number, updated?: DojoUserStateApi, inserted?: DojoUserStateApi }>
}
