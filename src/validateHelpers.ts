...
export function validateBasketName(name: string) : string {
    ...
    if (!name || name.length > 1000)
        throw new ERR_DOJO_INVALID_BASKET_NAME()
    return name
}
...
export function validateCustomInstructions(s: string) : string {
    s = s.trim()
    if (!s || s.length > 2500)
        throw new ERR_DOJO_INVALID_CUSTOM_INSTRUCTIONS()
    return s
}
...
export function validateOutputDescription(s: string) : string {
    s = s.trim()
    if (!s || s.length > 255)
        throw new ERR_DOJO_INVALID_OUTPUT_DESCRIPTION()
    return s
}
...
export function validateSecondsSinceEpoch(time: number) : Date {
    ...
    if (!date || date.getTime() / 1000 !== time || time < 1600000000 || time > 100000000000)
        throw new ERR_DOJO_INVALID_TIME()
    return date
}
...
export function validateTxLabel(label: string) : string {
    ...
    if (!label || label.length > 150)
        throw new ERR_DOJO_INVALID_TX_LABEL()
    return label
}
...
export function validateOutputToRedeem(output: DojoOutputToRedeemApi) {
    if (typeof output.index !== 'number')
        throw new ERR_DOJO_INVALID_REDEEM('index property is missing or invalid')
    if (typeof output.unlockingScriptLength !== 'number')
        throw new ERR_DOJO_INVALID_REDEEM('unlockingScriptLength property is missing or invalid')
}
...
export function validatePaymail(paymailHandle: string) {
  ...
  if (!re.test(paymailHandle) || paymailHandle.length > 128)
    throw new ERR_DOJO_INVALID_PAYMAIL_HANDLE('The paymail handle must follow e-mail address formatting rules and not be longer than 128 characters.')
}
